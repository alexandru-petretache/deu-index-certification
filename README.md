...
## Tree extremes (Prop:trees)

`trees_check.py` certifies the shared-tree-extremes proposition: over **all** trees on
`n` vertices (generated by nauty's `gentreeg`), both `DEU` and `DSO` are minimized uniquely
by the path and maximized uniquely by the star. Trees are cheap, so this runs far past
`n = 10` (e.g. all 123,867 trees on 18 vertices in well under a second), confirming the
extremal claim and its uniqueness for both indices simultaneously.
...
| `trees_check.py`      | tree extremes: path=min, star=max for DEU & DSO (needs `gentreeg`) |
...
# nauty (system dependency for enumerate_check.py and trees_check.py):
#   Debian/Ubuntu:  apt-get install nauty     # provides `nauty-geng`, `nauty-gentreeg`
...
# Tree extremes — path=min, star=max for DEU and DSO over all trees
python3 trees_check.py --nmax 18
```
...
| `trees_check.py` (path=min, star=max, both indices)  | shared tree extremes (Prop:trees)                |
